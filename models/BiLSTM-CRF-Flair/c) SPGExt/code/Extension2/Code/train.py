...
class Config():
    def __init__(self):
        ...
        # garantizar UNK en vocabulario de palabras
        if UNK not in vocab_words:
            if isinstance(vocab_words, set):
                vocab_words.add(UNK)
            else:
                vocab_words.append(UNK)

        # garantizar UNK en vocabulario de etiquetas
        if UNK not in vocab_tags:
            if isinstance(vocab_tags, set):
                vocab_tags.add(UNK)
            else:
                vocab_tags.append(UNK)
        ...
        for sent in all_train_sentences:
            ...
            for i, (word, label, startIndex, postag) in enumerate(sent):
                w = get_processing_word(
                    word,
                    self.vocab_words,
                    self.vocab_chars,
                    lowercase=True,
                    chars=True,
                    allow_unk=True      # palabras -> permitir UNK
                )
                t = get_processing_word(
                    label,
                    self.vocab_tags,
                    lowercase=False,
                    chars=False,
                    allow_unk=True      # etiquetas -> permitir UNK
                )
        ...
        # ==== 5. Embeddings ====
        self.embeddings = get_fasttext_vectors('../vocab_embeddings_flair.npz')
